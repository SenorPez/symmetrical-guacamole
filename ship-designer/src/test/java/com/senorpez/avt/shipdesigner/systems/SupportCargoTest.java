...
class SupportCargoTest {
    ...
    private SupportCargo instance;

    @BeforeEach
    void setUp() {
        when(ship.getNonSupplyCrew()).thenReturn(9.25d);
        instance = new SupportCargo(ship, 16, cargoArmorLevel, STANDARD);
    }
    ...
    @Test
    void getName() {
        String expectedValue = "Support Cargo";
        assertEquals(expectedValue, instance.getName());
    }

    @ParameterizedTest
    @MethodSource("quantityProvider")
    void getQuantity(final int cruiseDuration, final int expectedValue) {
        instance.setCruiseDuration(cruiseDuration);
    ...
    private static Stream<Arguments> quantityProvider() {
        return Stream.of(
                arguments(4, 0),
                arguments(8, 1),
                arguments(12, 2),
